...
- Slack channel threadを会話履歴として渡す。
- 外部repo変更はしない。
...
承認はSlack interactive buttonで行い、依頼元threadまたはoperations channel threadに残す。
...
- retry可能か判定する。
- retry不能なら依頼元threadまたはoperations channel threadでownerに相談する。
