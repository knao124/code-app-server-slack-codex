...
Codex-based Agentは「ツール」ではなくSlack上の常駐メンバーとして振る舞う。
...
Slackチャンネル上の新規メッセージは、原則としてCodex/Agentへの明示mentionがある場合だけ処理対象にする。

処理対象になるもの:

- 特定Agentへのmention
- 汎用Codex botへのmention
- slash command
- Agentが開始済みのthreadへの返信。root messageのmentionを引き継ぐ扱いにする。
- schedulerや内部jobが生成したevent

処理対象にしないもの:

- Agentへのmentionがない通常のチャンネル投稿
- 許可チャンネル内でも、Agentに話しかけていない雑談や人間同士の作業会話
- channel default Agentが存在するだけの新規投稿

`project_awareness` は許可されたチャンネルの状態把握に使えるが、mentionのないメッセージから作業ジョブやSlack返信を自動生成しない。日報や要約へ含める場合も、設定で許可された範囲に限定する。
...
- 他メンバーの日報や進捗の把握
- ブロッカー検知
- mentionされた依頼や日報に使うためのチャンネル状態把握
...
Agentの日報は、実行ジョブ、相談、レビュー、未完了タスクから自動生成する。
...
check mention gate
if not actionable, ack and stop
enqueue job
ack
